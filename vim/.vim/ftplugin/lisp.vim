...
" key: buffer-code / value: buffer-repl
let s:buf_map = {}

function! StartRepl()
    belowright vertical let term_buf = term_start("rlwrap sbcl --noinform", {"term_finish": "close"})
    execute "normal! \<c-w>p"
    let s:buf_map[ bufnr() ] = term_buf
endfunction

function! Repl()
    let msg = GetSelectedString() . "\<cr>"
"    let msg = ConvertOneLine(msg)
    call term_sendkeys(s:buf_map[ bufnr() ], msg)
endfunction
...
"function! ConvertOneLine(msg)
"    return substitute(a:msg, '\n\s\+', " ", "g")
"endfunction
